    ...
    Object.values(features).forEach((feature, i) => {
        ...
        let icon = feature.iconSmall || feature.icon;
            ...
            if (id == featureID) {
                ...
                $("#wrapper .features .small svg").removeClass("highlighted");
                $("#wrapper .features .small [data-id='"+id+"']").addClass("highlighted");

                $("#wrapper .features .selected .icon").html(icon);
                $("#wrapper .features .selected h4").html(feature.name[lang]);
                $("#wrapper .features .selected p").html(feature.desc[lang]);
            }
    });
}

$("#wrapper .features .small").html("");
...
Object.values(features).forEach((feature, i) => {
    ...
    if (!icon.startsWith("<svg")) {
        icon = '<svg viewBox="0 0 24 24" data-id="'+id+'" onclick="displayFeature(\''+id+'\');"><path d="'+icon+'"/></svg>';
    } else {
        icon = icon.replace("<svg", '<svg data-id="'+id+'" onclick="displayFeature(\''+id+'\');"');
    }

    $("#wrapper .features .small").append(icon);
});
